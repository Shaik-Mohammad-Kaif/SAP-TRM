...
import { FiUser, FiSun, FiMoon, FiTrash2, FiDownload, FiCheck, FiSettings, FiLogOut } from "react-icons/fi";
...
export default function App() {
  ...
  const [user, setUser] = useState(() => JSON.parse(localStorage.getItem("arco_user")));
  const [authState, setAuthState] = useState(user ? "loggedIn" : "login"); // 'login', 'signup', 'loggedIn'
  ...
  // REMOVED: localStorage sync causes race conditions with login
  // Chat history is now managed entirely through database persistence
  ...
  const fetchChatHistory = async () => {
    if (!user) return;
    try {
      const res = await fetch(`${settings.apiEndpoint}/chats?user_id=${user.id}`);
      const data = await res.json();
  ...
  const loadChat = (chatId) => {
    const selectedChat = chatHistory.find(c => String(c.id) === String(chatId));
    if (selectedChat) {
      setCurrentChatId(chatId);
      // Use messages from chatHistory state (already loaded from /chats endpoint)
      if (selectedChat.messages && Array.isArray(selectedChat.messages)) {
        setMessages(selectedChat.messages);
      } else {
        setMessages([]);
      }
      setInput(""); // Clear any leftover input from previous chat
      setActiveView("Dashboard");
    }
  };
  ...
  const deleteChat = async (chatId) => {
    try {
      // Delete from backend database (permanent deletion)
      const res = await fetch(`${settings.apiEndpoint}/chats/${chatId}?user_id=${user.id}`, {
        method: "DELETE"
      });
      const data = await res.json();
  ...
  const processMessage = async (text, baseMessages) => {
    ...
    // REMOVED: Frontend chat creation causes duplicates
    // Chats are automatically saved by backend /ask endpoint
    // and loaded from database on refresh or after sending message
    // setChatHistory(prev => {
    //   const existingIndex = prev.findIndex(c => c.id === currentChatId);
    //
    //   if (existingIndex >= 0) {
    ...
    //   }
    // });
    // Note: Chat is automatically saved to database by /ask endpoint

    const conversationHistory = [];
    // Increased history window for better memory
    const recentMessages = baseMessages.slice(-15);
    ...
    try {
      const response = await fetch(`${settings.apiEndpoint}/ask`, {
        ...
        body: JSON.stringify({
          ...
          user_id: user?.id  // Added for chat history persistence
        })
      });
      ...
      // Reload chats from backend to update sidebar without duplicates
      if (user) {
        try {
          const chatsResponse = await fetch(`${settings.apiEndpoint}/chats?user_id=${user.id}`);
      ...
      // Removed: setChatHistory update and fetchChatHistory - chats reload from backend above
    } catch {
      setIsTyping(false);
  ...
  const handleLogout = () => {
    ...
    // Clear all chat history keys (both global and user-specific)
    Object.keys(localStorage).forEach(key => {
      if (key.startsWith("arco_chat_history")) {
  ...
  const handleLoginSuccess = async (userData) => {
    ...
    // Clear ALL old localStorage chat data first
    Object.keys(localStorage).forEach(key => {
      if (key.startsWith("arco_chat_history")) {
        localStorage.removeItem(key);
      }
    ...
    // Load user's chat history ONLY from backend
    try {
      const response = await fetch(`${settings.apiEndpoint}/chats?user_id=${userData.id}`);
      const data = await response.json();
      if (data.chats && Array.isArray(data.chats)) {
        setChatHistory(data.chats);
      } else {
        setChatHistory([]); // Start with empty if no chats
      }
    } catch (e) {
      console.error("Failed to load chat history:", e);
      setChatHistory([]); // Start with empty on error
    }
  };
